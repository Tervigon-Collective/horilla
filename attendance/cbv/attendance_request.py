...
def change_accessibility(request, instance: object = None, *args, **kwargs) -> bool:
    """
    Who may act on this attendance request.

    Must match the rule the approve/reject views enforce, otherwise the row
    renders an action the backend then refuses. `is_reportingmanager()` was
    wrong here: it is true for anyone who manages *somebody*, so every
    reporting manager saw actions on every employee's request.
    """
    from employee.cbv.accessibility import can_manage_employee_action

    if instance is None:
        return False
    if request.user == instance.employee_id.employee_user_id:
        return True
    return can_manage_employee_action(
        request, instance.employee_id, "attendance.change_attendance"
    )
